import { App, Route, Response, useRoute, serve, RouteGroup, Middleware, useContext } from "react-serve-js";
import { db } from "./db";
import { desc, eq, count } from "drizzle-orm";
import { InsertTodo, todos } from "./db/schema";
import fs from "fs";
import path from "path";
import { authMiddleware, getCurrentUser, login, logout, register } from "./auth";
const users = [
      ...
      <RouteGroup prefix="/api">
        ...
        <RouteGroup prefix="/auth">
          ...
          <Route path="/logout" method="POST">
            {async () => {
              const { body } = useRoute();
              const user = await logout(body.token);
              return <Response status={200} json={user} />;
            }}
          </Route>
